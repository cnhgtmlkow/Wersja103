...
public class gameController {
    ...
    void pass(ActionEvent event) {
        if (czyZagranoKarta==false) { //zapobiega oszukiwaniu gracza; dodaje karte dla gracza ktory wczesniej jej nie dal
...
//            btnKartaNPC8.setGraphic(k);
    ...
    /*public void kartyOczekujacenpc() {
        if (czyZagranoKartanpc==false) {
            int ileKartManpc=npc.getKarty().size();
            if (ileKartManpc < 8) {
                ArrayList<Karta> kartynpc = npc.getKarty();
                ArrayList<Karta> oczekujace = stol.getOczekujace();
                Karta nowaKarta2 = stol.getOczekujace().get(0);
                oczekujace.remove(nowaKarta2);
                kartynpc.add(nowaKarta2);
                npc.setKarty(kartynpc);
                stol.setOczekujace(oczekujace);
            } else {
                System.out.println("Max liczba kart");
            }
